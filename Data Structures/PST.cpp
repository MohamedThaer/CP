...
template<class node, bool opt = false>
class PST {
private:
    int64_t LX, RX;
    ...
    int update(int x, int64_t lx, int64_t rx, int64_t l, int64_t r,int64_t val) {
    ...
    pair<int, node> find(int x, int64_t lx, int64_t rx, node *last, int64_t l, int64_t r, int dir, const auto &F) {
        ...
        auto [idx, nd] = (dir
                              ? find(left[x], lx, mid, last, l, r, dir, F)
                              : find(right[x], mid + 1, rx, last, l, r, dir, F));
        if (~idx) return {idx, nd};
        return dir ? find(right[x], mid + 1, rx, &nd, l, r, dir, F) : find(left[x], lx, mid, &nd, l, r, dir, F);
    }

public:
    PST(int64_t l = INT_MIN, int64_t r = INT_MAX) : LX(l), RX(r) {
        create_node(roots[0]);
    }
    ...
    void update(int time, int64_t l, int64_t r, int64_t val) {
