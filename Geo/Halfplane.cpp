...
struct line {
    ...
    static line bisector(line l1, line l2, bool interior) {
        ...
        return {
            l2.v / magnitude(l2.v) + l1.v / magnitude(l1.v) * sign,
...
struct segment {
    ...
    bool is_intersect(segment s) {
        return on_segment(s.a) || on_segment(s.b) || s.on_segment(a) || s.on_segment(b) || (
                   orient(a, b, s.a) != orient(a, b, s.b) && orient(s.a, s.b, a) != orient(s.a, s.b, b));
    }
    ...
    Type minimum_enclosing_rectangle() {
        ...
        while (i < n) {
            ...
            while (v[(mndot + 1) % n].dot_product(cur) <= v[mndot].dot_product(cur))
                mndot = (mndot + 1) % n;
            ans = min(
                ans, (Type) 2 * ((v[mxdot].dot_product(cur) / magnitude(cur) - v[mndot].dot_product(cur) /
                                  magnitude(cur)) + line(v[i], v[(i + 1) % n]).dist(v[j])));
            i++;
        }
        return ans;
    }
